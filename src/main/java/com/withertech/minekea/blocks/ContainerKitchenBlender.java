...
public class ContainerKitchenBlender extends Container
{
	private TileKitchenBlender te;
	private static final int PROGRESS_ID = 0;
	
	public ContainerKitchenBlender(IInventory playerInventory, TileKitchenBlender te)
	{
		this.te = te;
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
	{
		...
		if (slot != null && slot.getHasStack())
		{
			...
			if (index < TileKitchenBlender.SIZE)
			{
				if (!this.mergeItemStack(itemstack1, TileKitchenBlender.SIZE, this.inventorySlots.size(), true))
				{
					return ItemStack.EMPTY;
				}
			} else if (!this.mergeItemStack(itemstack1, 0, TileKitchenBlender.SIZE, false))
			{
				return ItemStack.EMPTY;
			}
