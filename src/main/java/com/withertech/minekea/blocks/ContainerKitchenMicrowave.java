...
public class ContainerKitchenMicrowave extends Container
{
	private TileKitchenMicrowave te;
	private static final int PROGRESS_ID = 0;
	
	public ContainerKitchenMicrowave(IInventory playerInventory, TileKitchenMicrowave te)
	{
		this.te = te;
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
	{
		...
		if (slot != null && slot.getHasStack())
		{
			...
			if (index < TileKitchenMicrowave.SIZE)
			{
				if (!this.mergeItemStack(itemstack1, TileKitchenMicrowave.SIZE, this.inventorySlots.size(), true))
				{
					return ItemStack.EMPTY;
				}
			} else if (!this.mergeItemStack(itemstack1, 0, TileKitchenMicrowave.SIZE, false))
			{
				return ItemStack.EMPTY;
			}
