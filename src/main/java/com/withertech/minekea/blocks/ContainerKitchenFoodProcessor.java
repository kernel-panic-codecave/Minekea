...
public class ContainerKitchenFoodProcessor extends Container
{
	private TileKitchenFoodProcessor te;
	private static final int PROGRESS_ID = 0;
	
	public ContainerKitchenFoodProcessor(IInventory playerInventory, TileKitchenFoodProcessor te)
	{
		this.te = te;
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
	{
		...
		if (slot != null && slot.getHasStack())
		{
			...
			if (index < TileKitchenFoodProcessor.SIZE)
			{
				if (!this.mergeItemStack(itemstack1, TileKitchenFoodProcessor.SIZE, this.inventorySlots.size(), true))
				{
					return ItemStack.EMPTY;
				}
			} else if (!this.mergeItemStack(itemstack1, 0, TileKitchenFoodProcessor.SIZE, false))
			{
				return ItemStack.EMPTY;
			}
